...
#x_shift and y_shift are used to place counter registers and adders in an adjacent LAB. t keeps track of the number of the RO being placed.
t = 0
x_shift = 0
y_shift = 0
...
with open('LocationAssignments.txt', 'w') as f:
	# create location assignments for ring oscillators
	...
	#create location assignments for counter registers. They will be place in the LAB to the right if possible. If not, they will be put in
	# the LAB above, below, or to the left.
	for j in range(xbegin, xend+1, xstep):
		for i in range(ybegin, yend+1, ystep):
			if x[(j,i)]["TYPE"] == "LAB":
				if x[(j+1,i)]["TYPE"] == "LAB":
					x_shift = 1
					y_shift = 0
				elif x[(j,i+1)]["TYPE"] == "LAB":
					x_shift = 0
					y_shift = 1
				elif x[(j-1,i)]["TYPE"] == "LAB":
					x_shift = -1
					y_shift = 0
				elif x[(j,i-1)]["TYPE"] == "LAB":
					x_shift = 0
				...
				for n in range(0,16):
					str1 = 'set_location_assignment FF_X' + str(j+x_shift) + '_Y' + str(i+y_shift) + '_N' + str(n*3 + 1)
					str2 =' -to counter[' + str(t) + '][' + str(n) + ']'
					f.write(str1 + str2 + '\n')
				t = t + 1	
	
#Place the adders. They will be placed in the same LAB as the counter registers.
	t = 0
	for j in range(xbegin, xend+1, xstep):
		for i in range(ybegin, yend+1, ystep):
			if x[(j,i)]["TYPE"] == "LAB":
				...
				for n in range(0,15):
					str1 = 'set_location_assignment LABCELL_X' + str(j+x_shift) + '_Y' + str(i+y_shift) + '_N' + str(n*3)
					str2 =' -to Add' + str(2+t) + '~' + str(n*4 + 1)
					f.write(str1 + str2 + '\n')
				t=t+1
